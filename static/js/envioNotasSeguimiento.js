...
function guardarDatosNotasSeguimiento() {
    ...
    secciones.forEach((seccion, index) => {
        const datosNotasSeguimiento = {
            nombrePaciente_Seguimiento: obtenerValor(`nombrePaciente_notasSeguimiento_${index}`),
            diagnostico_Seguimiento: obtenerValor(`diagnostico_notasSeguimiento_${index}`),
            ...
            sugerencias_Seguimiento: obtenerValor(`sugerencias_notasSeguimiento_${index}`),
            nombrefirma_Seguimiento: obtenerValor(`nombrefirma_notasSeguimiento_${index}`)
        };
...
function generarFolio() {
    ...
    const aleatorio = Math.floor(Math.random() * 100).toString().padStart(2, '0'); // Número aleatorio entre 00 y 99
...
function mostrarFolioEnCampos() {
    ...
    secciones.forEach((seccion, index) => {
        ...
        document.getElementById(`folio_notasSeguimiento_${index}`).value = folioSeccion;
    });
}

document.getElementById('miembroNotasSeguimiento').addEventListener('submit', function (event) {
                    ...
                    <div class="card-body">
                        <form class="row g-3 needs-validation" novalidate id="miembroNotasSeguimiento_${numSecciones}">
                            ...
                            <div class="form-group col-md-12 mt-1">
                                ...
                                <textarea class="form-control" name="notas" id="notas_notasSeguimiento_${numSecciones}" style="height: 100px" placeholder="Escribe aquí"></textarea>
                            </div>
                            <div class="form-group col-md-12 mt-1">
                                <label for="sugerencias_${numSecciones}">Sugerencias/observaciones:</label>
                                <textarea class="form-control" name="sugerencias" id="sugerencias_notasSeguimiento_${numSecciones}" style="height: 100px" placeholder="Escribe aquí"></textarea>
                            </div>
                            <div class="form-group col-md-12 mt-1">
                                <label for="nombrefirma_${numSecciones}">Nombre y firma del tratante:</label>
                                <textarea class="form-control" name="nombrefirma" id="nombrefirma_notasSeguimiento_${numSecciones}" style="height: 100px" placeholder="Escribe aquí"></textarea>
                            </div>
                        </form>
                    </div>
                </div>
...
function loadFormData() {
    const datosGuardados = JSON.parse(localStorage.getItem('datosNotasSeguimiento')) || { principal: {}, secciones: [] };
    ...
    if (Array.isArray(datosGuardados.secciones)) {
        datosGuardados.secciones.forEach((seccion, index) => {
            ...
            document.getElementById(`folio_notasSeguimiento_${index}`).value = seccion.folio_Seguimiento || '';
