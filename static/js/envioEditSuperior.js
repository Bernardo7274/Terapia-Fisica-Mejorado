...
document.getElementById('miembroFormSuperior').addEventListener('submit', function (event) {
    ...
    window.location.href = '/EdicionMiembroInferior'; // Redirige a la página de resumen
});
...
const establecerValor = (id, valor) => {
  ...
  if (id) {
    fetch('/consultarMiembrosSuperiores', {
      method: 'POST',
      headers: {
      ...
      .then(data => {
        ...
        establecerValor("observacionesMiembrosSuperiores", data.observacionesMiembrosSuperiores);
        establecerValor("palpacionMiembrosSuperiores", data.palpacionMiembrosSuperiores);
        establecerValor("dolorSuperiorMiembrosSuperiores", data.dolorSuperiorMiembrosSuperiores);
        establecerValor("dolorexplofisicaMiembrosSuperiores", data.dolorexplofisicaMiembrosSuperiores);
      })
      .catch(error => {
  ...
  if (id) {
    fetch('/consultarFuerzaMuscularMiembrosSuperiores', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      ...
      .then(data => {
        ...
        // Actualizar los campos en el DOM con los datos obtenidos
        data.forEach(fuerza => {
          const { movimiento, izquierdo, derecho } = fuerza;
  
          // Actualizar valores de los campos izquierdo y derecho
          establecerValor(`${movimiento}_izquierdo`, izquierdo);
          establecerValor(`${movimiento}_derecho`, derecho);
        });
      })
      .catch(error => {
        console.error("Error al consultar los datos:", error);
  ...
  if (id) {
    fetch('/consultarGoniometriaMiembrosSuperiores', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  if (id) {
    fetch('/consultarReflejosOsteotendinosos', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
  ...
  if (id) {
    fetch('/consultarPruebasEvaluacionesComplementarias', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
      },
      ...
      .then(data => {
        ...
        data.forEach((prueba, index) => {
          ...
          establecerValor(`Pruebas_${index + 1}_MiembrosSuperiores`, pruebaName);
          establecerValor(`Resultados y análisis_${index + 1}_MiembrosSuperiores`, resultado);
        });
      })
      .catch(error => {
        console.error("Error al consultar los datos:", error);
